<x-layouts.app :title="'New Requirement'">
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-2">
            <span class="inline-block h-5 w-1.5 rounded-full bg-gradient-to-b from-[#0018f9] to-[#38bdf8]"></span>
            <h2 class="m-0 text-[#0a1633]">New Requirement</h2>
        </div>
        <a href="{{ route('teacher.requirements') }}"
           class="rounded-lg bg-gradient-to-r from-[#0a1633] to-[#164aa8] px-4 py-2 font-semibold text-white no-underline shadow-[0_4px_14px_-4px_rgba(10,22,51,0.6)] transition hover:brightness-110">← Requirements</a>
    </div>
    ...
    <form method="POST" action="{{ route('teacher.requirements.store') }}" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
            <input type="text" name="title" placeholder="Title (e.g. Submit Birth Certificate)" required maxlength="200" value="{{ old('title') }}"
            ...
            <select name="requirement_type" class="futuristic-select px-2.5 py-1.5 text-[14px]">
                @foreach ($types as $value => $label)
                    <option value="{{ $value }}" {{ old('requirement_type') === $value ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
            <textarea name="description" rows="4" required maxlength="5000" placeholder="Describe what is needed and any instructions."
                      class="rounded-lg border border-[#0018f9]/20 bg-white p-2.5 text-[14px] shadow-sm outline-none transition focus:border-[#0018f9] focus:ring-2 focus:ring-[#0018f9]/15 sm:col-span-2">{{ old('description') }}</textarea>
        ...
        <p class="mt-1.5 text-[12.5px] text-[#0a1633]/55">Attachment is optional (max 5MB). Students can also receive requirements without a due date.</p>

        <label class="mt-4 flex cursor-pointer items-center gap-2.5 rounded-xl border p-3 transition {{ old('submission_required') ? 'border-[#0018f9]/60 bg-[#0018f9]/5' : 'border-slate-200 bg-white hover:border-[#0018f9]/30' }}">
            <input type="checkbox" name="submission_required" value="1" class="h-4 w-4 accent-[#0018f9]" {{ old('submission_required') ? 'checked' : '' }}>
        ...
        <div class="mt-5 rounded-xl border border-[#0018f9]/15 bg-[#f8fbff] p-3.5 text-[13px] text-[#0a1633]/75">
            This requirement will be sent to your <strong>{{ $studentCount }}</strong> currently approved student(s)
            ({{ $teacher->advisory_class ?? 'no advisory class set' }}).
        </div>
