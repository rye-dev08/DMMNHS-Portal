...
## 2. Current State of the Laravel App (root) — AFTER MIGRATION (DMMNHS-V.1.1→V.2.0)
...
- **Controllers:** `Auth\AuthController`, `PasswordController`, `PageController` (about/contact/submitContact), `Admin\*` (Dashboard, Account, TeacherApproval, EnrollmentSetting), `Teacher\*` (Dashboard, Subject, EnrollmentRequest, Grade, GradesOverview, Info), `Student\*` (Dashboard, StudentInfo, Schedule, Grade, Enrollment).
...
- **Admin accounts (V.1.5):** split into **Create Account** (`admin/accounts/create`) and **Manage Accounts** (`admin/accounts`) pages. Manage has role-filter tabs (all/student/teacher/admin), free-text search, pagination (15/page), inline activate/deactivate toggle, edit (role-specific profile fields), reset-password form, and cascade delete. Guards: cannot deactivate/delete self or the last active admin. `store` creates student profiles (active) or teacher profiles (inactive); role rule accepts `admin,teacher,student`.
- **Pending audit fixes (V.1.5+):** subject-logic fixes (per-student subjects vs per-teacher queries), GradeController authorization, `/teacher/subjects` JSON role gating, login/contact throttling, `graduated_students` writing, subject duplicate/delete checks, teacher dashboard null-guard, GradesOverview quarter filter — see Section 6.
...
1. **Subject-logic bugs:** `subjects` is per-student, but student Grades / teacher submit-grades JSON / GradesOverview query per-teacher → duplicate/wrong student mapping. Fix queries to use the student's own subject rows (by `student_id`).
2. **GradeController authorization:** `Student/GradeController` store + JSON endpoint lack authorization (any logged-in user could submit). Add role/ownership checks.
3. **`/teacher/subjects` JSON not role-gated:** currently `auth` only; should require the `teacher` role.
4. **No throttling:** login and contact POST routes should have `throttle` limits.
5. **Admin self-delete / last-admin guard:** (implemented in V.1.5 for accounts page) — also confirm deactivate path is guarded.
6. **`graduated_students` never written:** end-school-year should record graduating students.
7. **Subject duplicate check broken:** `orWhere('course_code', '')` matches empty; fix duplicate detection.
8. **Subject delete grade-block compares wrong ids:** fix comparison so grade-bearing subjects are correctly blocked.
9. **Teacher dashboard null-deref:** guard missing teacher/approval rows.
10. **GradesOverview ignores quarter:** filter by current semester.
